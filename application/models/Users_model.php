...
function get_supli_type_by_journal ($data) {
	...
	function update_journal_volume($data) {
	if(isset($data->id) && !empty($data->id)) {
		$query = $this->db->query("UPDATE wp_journal_volumes SET volume_name='".$data->volume_name."',updated_date='".date('Y-m-d')."' WHERE id=$data->id");
		} else {
		   $query = $this->db->query("INSERT INTO wp_journal_volumes (volume_name, updated_date, created_date) VALUES ('".$data->volume_name."','".date('Y-m-d')."','".date('Y-m-d')."')");
		}
		return $query;	
	}
