...
class Admin extends CI_Controller {
	...
	public function dashboard() {
		$this->load->model('Admin_model');	        
		$data = $this->Admin_model->get_count();
		if(is_array($data)){
	...
	public function get_categories() {
		...
		$this->load->model('Admin_model');	
		$data = $this->Admin_model->get_categories($search_str);
		if(is_array($data)){
			echo json_encode($data);
		}
		//echo $data;
	...
	public function get_journals() {
		...
		$search_str = (isset($obj->search_value) && !empty($obj->search_value))?$obj->search_value:"";
		$this->load->model('Admin_model');	
		$data = $this->Admin_model->get_journals($search_str);
		if(is_array($data)){
			echo json_encode($data);
		}
		//echo $data;
	...
	public function get_journals_archives() {
		$this->load->model('Admin_model');	
		$data = $this->Admin_model->get_journals_archives();
		//$journal_data = $this->Admin_model->get_journals_and_categories();
	...
	public function get_archives_by_journal() {
		...
		$data = $this->Admin_model->get_archives_by_journal($journal_id,$archive_in);
		if(is_array($data)){
			echo json_encode($data);
		}
	}
	public function get_main_category() {
		...
		$data = $this->Admin_model->get_main_category($cat_id);
		if(is_array($data)) {
			echo json_encode($data);
		}
		//echo $data;
	...
	public function get_latest_update() {
		...
		$data = $this->Admin_model->get_latest_update($l_id);
		if(is_array($data)) {
			echo json_encode($data);
		}		
	...
	public function insert_main_category() {
		...
		if($cat_id) {
			$data = $this->Admin_model->insert_main_category($cat_id,$cat_name);			
			if($data){
	...
	public function insert_latest_update() {
		...
		if($id) {
			$data = $this->Admin_model->insert_latest_update($id,$latest_update_description);			
			if($data){
	...
	public function insert_journal() {			
		...
		$journal_id = $obj->id;	
		if($journal_id) {
			$data = $this->Admin_model->insert_journal($obj);			
			if($data){
				$status = array('status' => true,"message" => 'Journal Edited Successfully','row_id'=>$journal_id);
			}
		} else {
			$data = $this->Admin_model->insert_journal($obj);			
			if($data) {
				$status = array('status' => true,"message" => 'Journal Added Successfully','row_id'=>$journal_id);			
			}
	...
	public function get_LatestArticles() {
		...
		$this->load->model('Admin_model');	
		$data = $this->Admin_model->get_LatestArticles($search_str);
		if(is_array($data)){
			echo json_encode($data);
		}
		//echo $data;
	...
	public function get_Testimonials() {
		$this->load->model('Admin_model');	
		$data = $this->Admin_model->get_Testimonials();
		if(is_array($data)){
			echo json_encode($data);
		}
		//echo $data;
...
public function get_supliTypes() {
		$this->load->model('Admin_model');	
		$data = $this->Admin_model->get_supliTypes();
		if(is_array($data)){
			echo json_encode($data);
		}
		//echo $data;
...
public function get_Suplitype() {		
		...
		//$data = $this->Admin_model->get_Suplitype($sid);	
		$data['s_info'] = $this->Admin_model->get_supliType($sid);
		$data['j_info'] = $this->Admin_model->get_journals("");
		if(is_array($data)) {
			echo json_encode($data);
		}
	}
	public function get_supli_type_by_journal () {
		...
		if(isset($obj->id) && !empty($obj->id)) {
			$this->load->model('Admin_model');	
			$data = $this->Admin_model->get_supli_type_by_journal($obj->id);
			if(is_array($data)){
				echo json_encode($data);
			}	
		}
	...
	public function get_SubmitManuscript() {
		$this->load->model('Admin_model');	
		$data = $this->Admin_model->get_SubmitManuscript();
		if(is_array($data)){
			echo json_encode($data);
		}
		//echo $data;
	...
	public function get_Home_Latest_Updates() {
		$this->load->model('Admin_model');	
		$data = $this->Admin_model->get_Home_Latest_Updates();
		if(is_array($data)){
			echo json_encode($data);
		}
	...
	public function get_journal() {
		...
		$data = $this->Admin_model->get_jounal($journal_id);
		if(is_array($data)) {
			echo json_encode($data);
		}
	}
	public function get_journalPage() {
		...
		$data['post_info'] = $this->Admin_model->get_journalPage($page_id);
	...
	public function get_latest_Article() {
		...
		$data['article_info'] = $this->Admin_model->get_latest_Article($article_id);		
		$data['journal_info'] = $this->Admin_model->get_journals_and_categories();
		if(is_array($data)) {
			echo json_encode($data);
		}
	...
	public function get_only_journals() {
		$this->load->model('Admin_model');
		$data = $this->Admin_model->get_journals_and_categories();
		if(is_array($data)) {
			echo json_encode($data);
		}
	}
	public function get_Testimonial() {
		$this->load->model('Admin_model');
		$article_id = $this->input->get('id');
		$data = $this->Admin_model->get_Testimonial($article_id);	
		if(is_array($data)) {
			echo json_encode($data);
		}
	}
	...
	public function updateTestimonial() {
		...
		if(isset($obj->id) && !empty($obj->id)) {
			$data = $this->Admin_model->updateTestimonial($obj);			
			if($data){
	...
	public function get_new_eb_member() {
		...
		$this->load->model('Admin_model');			
		$data['eb_info'] = $this->Admin_model->get_new_eb_member($id);
		$data['journal_info'] = $this->Admin_model->get_journals_and_categories();
		...
		if(isset($obj->id) && !empty($obj->id)) {
			$data = $this->Admin_model->updateSuplitype($obj);			
			if($data){
	...
	public function upload_files() {
        ...
        foreach ($_FILES as $key => $value) {        		
        	if($value['name']) {        		
		        ...
		        {		        	
		            $data = array('upload_data' => $this->upload->data(), 'success' => true);		            
	...
	private function _save_uploaded_to_db($data) {
		$this->load->model('Admin_model');	
		$result = $this->Admin_model->save_uploaded_to_db($data);
		if(is_array($result)){
...
public function get_new_eb_members_by_journal() {
		...
		$data = $this->Admin_model->get_new_eb_members_by_journal($journal_id);
		if(is_array($data)){
			echo json_encode($data);
		}
	}
...
public function deleteJournalPost() {
		...
		$obj=json_decode(file_get_contents('php://input'));		
		if(isset($obj->id) && !empty($obj->id)) {
			$data = $this->Admin_model->deleteJournalPost($obj->id);			
			if($data){
...
public function deleteLatestArticle() {
	...
	$obj=json_decode(file_get_contents('php://input'));		
	if(isset($obj->id) && !empty($obj->id)) {
		$data = $this->Admin_model->deleteLatestArticle($obj->id);			
		if($data){
...
public function deleteTestimonial() {
	...
	$obj=json_decode(file_get_contents('php://input'));		
	if(isset($obj->id) && !empty($obj->id)) {
		$data = $this->Admin_model->deleteTestimonial($obj->id);			
		if($data){
...
public function deleteFulltextarticle() {
	...
	$obj=json_decode(file_get_contents('php://input'));		
	if(isset($obj->id) && !empty($obj->id)) {
		$data = $this->Admin_model->deleteFulltextarticle($obj->id);			
		if($data){
			$status = array('status' => true,"message" => 'Testimonial Deleted Successfully');
		}
	} 
...
public function update_fulltext_article() {
		...
		if(isset($obj['ID']) && !empty($obj['ID'])) {			
			$data = $this->Admin_model->update_fulltext_article($obj);			
			if($data){
	...
	public function get_fulltext_articles() {
		...
		$search_str = (isset($obj->search_value) && !empty($obj->search_value))?$obj->search_value:"";
		$this->load->model('Admin_model');	
		$data = $this->Admin_model->get_fulltext_articles($search_str);
		if(is_array($data)){
			echo json_encode($data);
		}
	}
public function get_FulltextArticle() {
		...
		$id = $this->input->get('id');				
		$data['j_info'] = $this->Admin_model->get_journals("");		
		$data['fulltext_info'] = $this->Admin_model->get_FulltextArticle($id);		
		if(is_array($data)) {
			echo json_encode($data);
		}	
	}
...
public function getUploadedFiles() {		
	$this->load->model('Admin_model');	
	$data = $this->Admin_model->getUploadedFiles();
	if(is_array($data)){
		echo json_encode($data);
	}
	//echo $data;
